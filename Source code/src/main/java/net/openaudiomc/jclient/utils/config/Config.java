...
public class Config {
	...
	public void load() {
		...
		if(!configFile.exists()) {
			...
			this.messages.setvolumefail(config.getString("messages.setvolumefail"));
			this.messages.setvolume(config.getString("messages.setvolume"));
			...
			if(config.get("storage.speakerlocations") != null) {
				for (String speakerLocationsConfig : config.getConfigurationSection("storage.speakerlocations").getKeys(false)) {
					...
					speakerLocation.setX(Double.parseDouble(config.getString("storage.speakerlocations." + speakerLocationsConfig + ".x")));
					speakerLocation.setY(Double.parseDouble(config.getString("storage.speakerlocations." + speakerLocationsConfig + ".y")));
					speakerLocation.setZ(Double.parseDouble(config.getString("storage.speakerlocations." + speakerLocationsConfig + ".z")));
	...
	private void saveDefaultConfig() {
		...
		this.messages.setvolumefail("&4Invalid volume! please use a number between 0 and 100");
		this.messages.setvolume("&2Your volume has been set to %volume%");
	...
	public ConfigCommands getCommands() {
		return commands;
	}
}
